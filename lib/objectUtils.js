...
async function getVertices(gl, objPath){
...
async function position(gl, program, objRotationAngle, translateVector3, scaleVector3, canvas, eye, look, objRotationAfterTranslationAngle){
    ...
    if(objRotationAfterTranslationAngle !== null && objRotationAfterTranslationAngle !== undefined){
        ...
        setMat4Uniform(program,rotateAfterTransMatrix, 'mRotateAfterTrans', gl);
    }
...
async function drawFirefly(posCounter, fireflyFbProgram,fireflyProgram, fireflyVertices,canvasFireflyVertices, eye,look,fireflyTexture,positions, gl,fb, drawOnlyAt) {
    // bloom
    ...
    await fireflyFb.draw(drawOnlyAt)
    ...
    await canvasFirefly.draw(drawOnlyAt);
}