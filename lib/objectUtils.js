...
function initFogForProgram(program){
    ...
    let fogNearValue = elements.fogNearInput.value/1000.;
    let fogFarValue = elements.fogFarInput.value/1000.;
...
function getFireflyPosition() {
    ...
    for (let i = 0; i < 200; i++) {
        pos1.push(calcFireflyPosition(i));
    }
    for (let i = 200; i > 0; i--) {
        pos1.push(calcFireflyPosition(i));
    }
    ...
    for (let i = -100; i < 100; i++) {
        pos2.push(calcFireflyPosition2(i));
    }
    for (let i = 100; i > -100; i--) {
        pos2.push(calcFireflyPosition2(i));
    }
...
function createLighting(ambiente, diffuse, specular, direction) {
    const lighting = new Lighting();
    
    lighting.ambient = new Color(ambiente[0],ambiente[1],ambiente[2],ambiente[3]);
    lighting.diffuse = new Color(diffuse[0],diffuse[1],diffuse[2],diffuse[3]);
    lighting.specular = new Color(specular[0],specular[1],specular[2],specular[3]);
    lighting.direction = direction;

    return lighting;
}

function bindVideoTexture(texture, video) {
    gl.bindTexture(gl.TEXTURE_2D, texture);
