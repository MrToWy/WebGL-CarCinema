...
function rotateY(out, input, rad) {
    out[0] = input[0] * Math.cos(rad) + input[8] * -Math.sin(rad);
    out[1] = input[1] * Math.cos(rad) + input[9] * -Math.sin(rad);
    out[2] = input[2] * Math.cos(rad) + input[10] * -Math.sin(rad);
    out[3] = input[3] * Math.cos(rad) + input[11] * -Math.sin(rad);
    out[4] = input[4];
    out[5] = input[5];
    out[6] = input[6];
    out[7] = input[7];
    out[8] = input[0] * Math.sin(rad) + input[8] * Math.cos(rad);
    out[9] = input[1] * Math.sin(rad) + input[9] * Math.cos(rad);
    out[10] = input[2] * Math.sin(rad) + input[10] * Math.cos(rad);
    out[11] = input[3] * Math.sin(rad) + input[11] * Math.cos(rad);
    out[12] = input[12];
    out[13] = input[13];
    out[14] = input[14];
    out[15] = input[15];
